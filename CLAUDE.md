...
- 기록된 예외: 진행상황 한글 요약 / 작업 대기열 (09-03), 개발시스템 전체 수정 (09-03),
  ...
  빌드·QA 게이트 배선 (09-15 - '버그가 없는 선으로 너가 직접 작업해'. `CODEX-BUILDGATE1`, `CODEX-QAGATE1`, `CODEX-QUALITYSTEP1`.
  새 판정 로직은 쓰지 않았다: 판정은 이미 `quality_gates.py` 에 다 있었고 부르는 곳이 한 군데도 없었다),
  작업판 저장 병합 (09-15 - '작업판 덮어쓰기 문제'. `CLAUDE-BOARDSAVE1`.
  `save()` 가 통째로 덮어써서 실행 중 추가된 작업 3건이 사라진 적이 있다),
  기기 실행 파이프라인 (09-15 - '파이프라인 만들어'. `CLAUDE-DEVICE1`.
  adb 로 APK 를 실제 기기에 올려 실행·촬영. 이전까지 이 저장소에는 게임 화면을
  본 주체가 없었다).
