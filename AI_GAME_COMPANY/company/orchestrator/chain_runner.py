...
def chain_tasks(board: TaskBoard, prefix: str) -> list[Task]:
    """The plan's tasks in DEPENDENCY order.

    Not id order. That worked only while ids carried their position -
    PLAN-game02-01, -02 - and broke the moment plans used readable names:
    GAME02-ARCH sorts before GAME02-DESIGN, so the chain tried the second
    step first and run_task refused it for an unmet dependency, twice, and
    the repeated-failure rule stopped the whole plan. depends_on is what
    actually states the order, so that is what is read.

    A cycle, or a dependency outside this plan, leaves the remaining tasks in
    their board order rather than raising: the chain's job is to run what it
    can and report, and run_task still refuses anything genuinely unmet.
    """
    ...
    while remaining:
        ready = [t for t in remaining
                 if all(d in placed or d not in inside for d in t.depends_on)]
        if not ready:
            # Cycle or a dependency this plan does not contain. Board order
            # for the rest, and let run_task have the final say.
...
def run_chain(board: TaskBoard, prefix: str,
    ...
    while index < len(tasks):
        ...
        parked = task.extra.get("review_note", "")

        # REVIEW with no review_note is teamwork.run_task's "this step
        # succeeded" - a finished step, not a question for anybody.
        if task.status == DONE or (task.status == REVIEW and not parked):
            index, attempt = index + 1, 1
            continue

        if task.status == REVIEW and parked:
            if task.id in cleared:
                task.notes.append(
                    f"CONTINUE: 사람이 검토를 통과시켰습니다 ({parked}).")
                task.extra.pop("review_note", None)
                task.status = DONE
                index, attempt = index + 1, 1
                continue
        ...
        if action == REVIEW_WAIT:
            task.status = REVIEW
            # extra is Codex's own passthrough slot for keys the dataclass does
            # not name; it round-trips through the board untouched. Using it
            # means no new Task field, and a board written before this still
            # gains nothing.
            task.extra["review_note"] = why
