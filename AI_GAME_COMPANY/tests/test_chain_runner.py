...
class PauseAndReviewTests(unittest.TestCase):
    ...
    def test_an_unsure_agent_parks_the_task_for_a_person(self):
        ...
        self.assertIn("확신이 없다", target.tasks[0].extra["review_note"])
    ...
    def test_a_waiting_task_is_not_re_run_on_the_next_call(self):
        ...
        target = board_with("game_director")
        target.tasks[0].status = REVIEW
        target.tasks[0].extra["review_note"] = "확인 필요"
        calls: list[str] = []
    ...
    def test_continuing_steps_over_the_task_without_re_running_it(self):
        ...
        self.assertTrue(result.finished)
        self.assertEqual(["PLAN-t-02"], calls)
        self.assertEqual("", target.tasks[0].extra.get("review_note", ""))

    def test_approving_one_task_does_not_clear_another(self):
        target = board_with("game_director", "gameplay_engineer")
        for task in target.tasks:
            task.status = REVIEW
            task.extra["review_note"] = "확인 필요"
        ...
        self.assertEqual("확인 필요", target.tasks[1].extra["review_note"])


class OrderingTests(unittest.TestCase):
    """The chain runs in dependency order, not id order."""

    def test_readable_ids_still_run_in_the_right_order(self):
        # GAME02-ARCH sorts before GAME02-DESIGN. Sorting by id ran the
        # second step first, run_task refused it for an unmet dependency
        # twice, and the repeated-failure rule stopped the whole plan.
        tasks = [
            Task(id="G-DESIGN", title="d", owner="codex", agent_role="game_director"),
            Task(id="G-ARCH", title="a", owner="codex", agent_role="game_director",
                 depends_on=["G-DESIGN"]),
            Task(id="G-BUILD", title="b", owner="codex", agent_role="game_director",
                 depends_on=["G-ARCH"]),
        ]
        board = TaskBoard(path=Path(tempfile.mkdtemp()) / "b.json", tasks=tasks)
        self.assertEqual(["G-DESIGN", "G-ARCH", "G-BUILD"],
                         [t.id for t in chain.chain_tasks(board, "G-")])

    def test_a_dependency_cycle_does_not_hang(self):
        tasks = [
            Task(id="G-A", title="a", owner="codex", depends_on=["G-B"]),
            Task(id="G-B", title="b", owner="codex", depends_on=["G-A"]),
        ]
        board = TaskBoard(path=Path(tempfile.mkdtemp()) / "b.json", tasks=tasks)
        self.assertEqual(2, len(chain.chain_tasks(board, "G-")))

    def test_a_dependency_outside_the_plan_is_not_treated_as_unmet(self):
        tasks = [Task(id="G-A", title="a", owner="codex",
                      depends_on=["CODEX-SOMETHING-ELSE"])]
        board = TaskBoard(path=Path(tempfile.mkdtemp()) / "b.json", tasks=tasks)
        self.assertEqual(["G-A"], [t.id for t in chain.chain_tasks(board, "G-")])
