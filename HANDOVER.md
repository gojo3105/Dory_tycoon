...
### 추가: 캐릭터 팔·다리 (2026-09-08 02:25 PC 기준, 전부 확인된 사실)

**Unity가 2D 옆모습 그림을 파츠로 자르고 걸음 애니메이션까지 만들었다.** Gemini 키
없이, 외부 서비스 없이 된다.

| 항목 | 값 | 근거 |
|---|---|---|
| 원본 | `Assets/Common/Art/Runner/player_side.png` (2D 옆모습) | 사용자가 업로드 |
| 자른 결과 | `body` 110x192, `arm_near` 39x47, `foot_near` 59x18, `foot_far` 43x13 | fork의 `rig/*.png` |
| 지문 | `reference_hash: 7315b5113edcc2e9` — 컨테이너의 원본 바이트와 **일치** | fork의 `rig.json` |
| 생성 시각 | 2026-09-08 02:25:08 | 같은 파일 |
| PlayMode | **11개 통과, 실패 0** | 실행 34147483869 |
| 컴파일 에러 | 0 | `Reports/errors/latest.txt` 02:33:01 |
| APK | **19.67 MB, sha256:8D7CC5ADFA5082DC**, 02:32:44 빌드 | `Reports/build-status/latest.txt` |

- 이전 APK는 17.32 MB / sha256:7F81E6F18E29EA94였다. 크기와 해시가 둘 다 달라졌으니
  **새로 만들어진 파일**이다. 2D 그림이 3D placeholder보다 커서 2.35 MB 늘었다.
- 관절 3개(`anchor`)가 컨테이너에서 눈으로 검토한 값과 소수 5자리까지 같다.
  파츠 픽셀도 눈으로 대조했다 — 같은 그림, 같은 컷, 메운 배 아래에 윤곽선도 있다.
- **APK는 있지만 아직 폰에 설치해서 보지는 않았다.** 걸음이 자연스러운지는
  기기에서 봐야 안다. 그게 다음 할 일이고, 그건 사람만 할 수 있다.
- 정면 컷 표(`FrontCuts`)는 지금 그림과 안 맞는다. 옆모습이 있으면 안 쓰이고,
  크기 가드가 경고를 띄운다. 정면으로 돌아갈 일이 생기면 다시 재야 한다.
